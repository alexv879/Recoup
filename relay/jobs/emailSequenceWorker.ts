...
async function processInvoice(invoice: Invoice): Promise<string | null> {
    ...
    for (const reminderLevel of levelsToCheck) {
        ...
        if (!alreadySent) {
            ...
            try {
                logger.info(`Sending ${reminderLevel.displayName} reminder for invoice ${invoice.invoiceNumber}`, {
                ...
                logger.info(`Successfully sent ${reminderLevel.displayName} reminder`, {
                    invoiceId: invoice.id,
                ...
                logger.error(`Failed to send ${reminderLevel.displayName} reminder`, {
                    invoiceId: invoice.id,
...
export async function runEmailSequenceWorker(): Promise<{
    ...
    try {
        ...
        logger.info(`Found ${snapshot.size} potentially overdue invoices to process`);
        ...
        logger.info('Email sequence worker completed', {
            processed,
            sent,
            skipped,
            failed,
        ...
        logger.error('Email sequence worker failed', {
            error: error instanceof Error ? error.message : String(error),
        });
        throw error;
    }
