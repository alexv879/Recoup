...
async function checkVerificationDeadlines(): Promise<VerificationCheckResult> {
    ...
    try {
        ...
        const claimsRef = collection(db, 'payment_claims');
        ...
        for (const claimDoc of snapshot.docs) {
            ...
            try {
                ...
                if (timeUntilDeadline <= 0) {
                    ...
                    // Track analytics
                    await trackEvent(claim.freelancerId, 'payment_claim_auto_rejected', {
                        claim_id: claimId,
...
async function sendVerificationReminder(claim: any, hoursRemaining: 24 | 6): Promise<void> {
    ...
    const userDoc = await getDocs(
        query(collection(db, 'users'), where('userId', '==', claim.freelancerId))
    );
    ...
    const freelancer = userDoc.docs[0].data();
  ...
  `;
    ...
    await sendReminderEmail({
        ...
        reminderType: isUrgent ? 'verification_urgent' : 'verification_24h',
    });

    // Track analytics
    await trackEvent(claim.freelancerId, 'verification_reminder_sent', {
