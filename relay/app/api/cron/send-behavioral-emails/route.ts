...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    for (const user of day1Users) {
      try {
        await sendTutorialEmail(user.email, user.name || 'there');
    ...
    for (const user of day3Users) {
      try {
        await sendSocialProofEmail(user.email, user.name || 'there');
    ...
    for (const user of day7Users) {
      try {
        await sendFeatureDeepDiveEmail(user.email, user.name || 'there');
    ...
    for (const user of day14Users) {
      try {
        const totalInvoiced = user.totalAmountInvoiced || 0;
        await sendUpgradePitchEmail(user.email, user.name || 'there', totalInvoiced);
    ...
    for (const user of inactiveUsers) {
      try {
        await sendInactiveUserEmail(user.email, user.name || 'there');
    ...
    for (const user of quotaLimitUsers) {
      try {
        await sendQuotaLimitEmail(
          ...
          user.collectionsUsedThisMonth || 0,
          user.collectionsLimitPerMonth || 1
        );
...
async function getUsersForDay1Tutorial() {
  ...
  return snapshot.docs
    ...
    .filter((user: any) => !user.emailsSent?.includes('tutorial_day1'));
}
...
async function getUsersForDay3SocialProof() {
  ...
  return snapshot.docs
    ...
    .filter(
      (user: any) =>
        user.subscriptionTier === 'free' && !user.emailsSent?.includes('social_proof_day3')
    );
}
...
async function getInactiveUsers() {
  ...
  return snapshot.docs
    ...
    .filter((user: any) => !user.emailsSent?.includes('re_engagement_inactive_recent'));
}
...
async function getUsersNearQuotaLimit() {
  const snapshot = await db
    .collection('users')
    .where('subscriptionTier', 'in', ['free', 'starter', 'pro'])
    .get();
  ...
  return snapshot.docs
    ...
    .filter((user: any) => {
      ...
      return percentageUsed >= 80 && !user.emailsSent?.includes('quota_limit_recent');
    });
}
...
async function markEmailSent(userId: string, emailType: string, day: number) {
  ...
  await db
    ...
    .doc(userId)
    .update({
      emailsSent: db.FieldValue.arrayUnion(emailKey),
