...
export async function POST(
    ...
    try {
        logApiRequest('POST', `/api/payment-claims/${params.id}/evidence`);
        ...
        // Get payment claim
        const claimRef = doc(db, 'payment_claims', claimId);
        ...
        const uploadResult = await uploadBytes(storageRef, Buffer.from(fileBuffer), {
        ...
        await trackEvent(userId, 'payment_evidence_uploaded', {
            claim_id: claimId,
        ...
        logApiResponse('POST', `/api/payment-claims/${params.id}/evidence`, 200, duration);
        ...
        const duration = Date.now() - startTime;
        logApiResponse('POST', `/api/payment-claims/${params.id}/evidence`, 500, duration);
...
export async function GET(
    ...
    try {
        logApiRequest('GET', `/api/payment-claims/${params.id}/evidence`);
        ...
        // Log response
        const duration = Date.now() - startTime;
        logApiResponse('GET', `/api/payment-claims/${params.id}/evidence`, 200, duration);
        ...
        const duration = Date.now() - startTime;
        logApiResponse('GET', `/api/payment-claims/${params.id}/evidence`, 500, duration);
        return handleApiError(error);
    }
}
