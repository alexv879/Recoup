    ...
    try {
        ...
        if (paidInvoices.length >= 5) {
            const paymentDays = paidInvoices.map((inv) =>
                differenceInDays(inv.paidAt!.toDate(), inv.sentAt!.toDate())
            );
        ...
        if (outstandingInvoices.length > 0) {
            ...
            const overdueCount = outstandingInvoices.filter(
                (inv) => inv.dueDate && inv.dueDate.toDate() < now
            ).length;
        ...
        if (collectionsInvoices.length >= 3) {
            ...
            const eligibleForCollections = outstandingInvoices.filter(
                (inv) => !inv.collectionsEnabled && inv.dueDate && differenceInDays(now, inv.dueDate.toDate()) >= 7
            );
        ...
        const upcomingInvoices = invoices.filter(
            (inv) =>
                ...
                differenceInDays(inv.dueDate.toDate(), now) <= 30
        );
...
function analyzeClientPatterns(invoices: Invoice[]) {
    ...
    invoices.forEach((inv) => {
        ...
        if (inv.status === 'paid' && inv.paidAt && inv.sentAt) {
            client.totalPaid += inv.amount;
            const days = differenceInDays(inv.paidAt.toDate(), inv.sentAt.toDate());
