...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!tier || !['starter', 'pro', 'business'].includes(tier)) {
      throw errors.badRequest('Invalid tier. Must be starter, pro, or business.');
    }

    // 4. Validate eligibility
    const eligibility = await validateFoundingEligibility(userId);
    if (!eligibility.isEligible) {
      throw errors.badRequest(eligibility.reason || 'Not eligible for founding member program');
    }
    ...
    if (!result.success) {
      throw errors.badRequest(result.reason || 'Failed to register as founding member');
    }
    ...
    return handleApiError(error, 'POST', '/api/founding-members/register');
  }
}
