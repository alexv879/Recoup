...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!WEBHOOK_SECRET) {
      ...
      throw errors.serverError('Webhook secret not configured');
    }
    ...
    if (!svix_id || !svix_timestamp || !svix_signature) {
      ...
      throw errors.badRequest('Missing svix headers');
    }
    ...
    return handleApiError(error, 'POST', '/api/webhooks/clerk');
  }
}
