...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        const rateLimit = await checkWebhookRateLimit({
            req,
            source: 'twilio',
            webhookType: 'voice-ai',
        });

        if (!rateLimit.allowed) {
            logError('Rate limit exceeded for Twilio voice-ai webhook', undefined, { correlationId });
        ...
        if (!validateWebhookOrigin(req)) {
            logError('CSRF: Invalid origin on webhook', undefined, { correlationId });
            return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
        }

        if (!validateContentType(req, ['application/x-www-form-urlencoded'])) {
            logError('CSRF: Invalid Content-Type', undefined, { correlationId });
        ...
        if (!isValid) {
            logError('Invalid Twilio signature on voice-ai webhook', undefined, { correlationId });
        ...
        if (!contextParam) {
            logError('Missing context parameter', undefined, { correlationId });
  ...
  <Connect>
    <Stream url="${websocketUrl}?context=${streamContext}">
      <Parameter name="context" value="${streamContext}" />
        ...
        logError('Voice AI webhook error', error, { correlationId });
        ...
        try {
            ...
            logError('Failed to store webhook for recovery', recoveryError, { correlationId });
        }
