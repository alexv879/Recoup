...
#### 4. Migration Script
- `scripts/migrate_stripe_plans.py` (Python script) — migrated from Node to Python to centralize migration tooling
...
```bash
cd relay
python3 scripts/migrate_stripe_plans.py --dry-run
```
...
python3 scripts/migrate_stripe_plans.py --execute
```
...
3. Re-run migration for specific user:
   ```bash
  python3 scripts/migrate_stripe_plans.py --execute --userId=user_xxx
   ```
