...
export async function updateUserAchievements(userId: string) {
    ...
    if (stats.onTimePercentage && stats.onTimePercentage >= 90) newBadges.push('reliable');
    if (stats.streak && stats.streak >= 7) newBadges.push('week_streak');
    if (stats.streak && stats.streak >= 30) newBadges.push('month_streak');
...
export async function calculateUserLevel(userId: string): Promise<number> {
    ...
    points += stats.streak || 0; // 1 point per day of streak
