...
export type PaymentEvent =
  ...
  | 'payment_evidence_downloaded' // Evidence file downloaded by admin
...
export type CollectionsEvent =
  ...
  | 'client_archived'             // Client archived (soft delete)
  | 'client_updated'              // Client information updated
  | 'collections_timeline_viewed'; // User viewed collections timeline
...
// System-level events (non-core): keyboard shortcuts, PDF exports
export type SystemEvent = 'keyboard_shortcut_used' | 'pdf_exported';

// Add to the master event union
export type AnalyticsEventFull = AnalyticsEvent | SystemEvent;
...
interface InvoiceProperties extends BaseEventProperties {
  ...
  client_name?: string;               // Client name for invoice_created
  time_to_first_invoice_minutes?: number; // Time from signup to first invoice
}
...
interface PaymentProperties extends BaseEventProperties {
  ...
  reason?: string;                    // payment_claim_auto_rejected
  hours_remaining?: number;           // verification_reminder_sent
  is_urgent?: boolean;                // verification_reminder_sent
  file_type?: string;                 // payment_evidence_uploaded, payment_evidence_downloaded
  file_size?: number;                 // payment_evidence_uploaded, payment_evidence_downloaded
  file_name?: string;                 // payment_evidence_uploaded, payment_evidence_downloaded
  paymentMethod?: string;             // payment_verification_claimed
  hasEvidence?: boolean;              // payment_verification_claimed
}
...
interface CollectionsProperties extends BaseEventProperties {
  invoice_id?: string;
  client_id?: string;                // client_added, client_archived
  ...
  pause_until?: string;               // Added to support escalation pause
  current_stage?: string;             // collections_timeline_viewed
  amount_due?: number;                // collections_timeline_viewed
}
...
interface VoiceProperties extends BaseEventProperties {
  ...
  has_client_data?: boolean;          // voice_invoice_submitted
  has_amount?: boolean;               // voice_invoice_submitted
  has_line_items?: boolean;           // voice_invoice_submitted
}
...
interface KeyboardShortcutProperties extends BaseEventProperties {
  shortcut?: string;
  context?: string;
}

interface PdfExportProperties extends BaseEventProperties {
  invoice_count?: number;
  total_invoiced?: number;
}

// Extend the EventProperties union to include system events
export type EventPropertiesExtended = EventProperties | KeyboardShortcutProperties | PdfExportProperties;
...
class AnalyticsClient {
   ...
   * Track an event
   */
  track(event: AnalyticsEventFull, properties?: EventPropertiesExtended) {
 ...
 * Track an event
 */
export function trackEvent(event: AnalyticsEventFull, properties?: EventPropertiesExtended) {
 ...
 */
export function useTrackMount(event: AnalyticsEventFull, properties?: EventProperties) {
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      trackEvent(event, properties);
    }
