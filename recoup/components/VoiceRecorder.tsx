...
// Accessibility: prefers-reduced-motion audit
...
export function VoiceRecorder({
  ...
  useEffect(() => {
    ...
    // Add keyboard shortcuts per research recommendations
    const handleKeyDown = (e: KeyboardEvent) => {
      // Spacebar to start/stop recording (when not typing in inputs)
      if (e.code === 'Space' && e.target === document.body && recordingState !== 'processing') {
        e.preventDefault();
  ...
  const startRecording = async () => {
    try {
      ...
      trackEvent('error_occurred', {
        ...
        severity: 'medium',
        trace_id: 'voice_recording_start_failed',
      });
    }
  };
  ...
  const transcribeAudio = async (audioBlob: Blob) => {
    ...
    try {
      ...
      trackEvent('voice_transcript_finalized', {
        latency_ms_avg: data.latency,
        has_client_data: !!data.invoiceData?.clientName,
        has_amount: !!data.invoiceData?.amount,
        has_line_items: !!(data.invoiceData?.lineItems && data.invoiceData.lineItems.length > 0),
      });
      ...
      trackEvent('error_occurred', {
        ...
        severity: 'medium',
        trace_id: 'voice_transcription_failed',
      });
    }
  };
  ...
  return (
    <div className={`bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      {/* Screen Reader Status Announcements per Research */}
      <div
        role="status"
        aria-live="assertive"
        aria-atomic="true"
        ...
        id="voice-status"
      >
        {recordingState === 'recording' && `Recording started. Speak now. Recording ${recordingDuration} seconds.`}
        {recordingState === 'processing' && 'Processing audio. Transcribing...'}
        {recordingState === 'success' && `Transcription complete. ${transcript ? `Transcript: ${transcript}` : ''}`}
        {recordingState === 'error' && `Error: ${error}`}
      </div>
      {/* Instructions */}
      {showInstructions && recordingState === 'idle' && (
        <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 border-b border-gray-200">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Tip: Press Spacebar to start recording, Escape to cancel
          </p>
        </div>
      )}
      ...
      <div className="p-6">
        <div className="flex flex-col items-center gap-4">
          {/* Microphone Button - FAB Style per Research */}
          <button
            onClick={recordingState === 'recording' ? stopRecording : startRecording}
            disabled={recordingState === 'processing'}
            className={`relative w-14 h-14 rounded-full flex items-center justify-center transition-all duration-200 ${recordingState === 'recording'
          ...
          >
            {/* Audio level visualization - Animated Bars per Research */}
                ...
                <div className="flex items-end space-x-1">
                  {Array.from({ length: 20 }, (_, i) => (
                    <div
                      ...
                      style={{
                        height: `${Math.max(4, audioLevel * 30 + Math.sin(Date.now() * 0.01 + i * 0.5) * 5)}px`,
          ...
          {transcript && recordingState === 'success' && (
            <div className="w-full bg-green-50 rounded-lg p-4 border border-green-200" aria-live="polite" aria-atomic="true" role="status">
