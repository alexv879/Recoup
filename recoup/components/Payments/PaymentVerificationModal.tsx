...
export default function PaymentVerificationModal({
    ...
    return (
        <AccessibleDialog
            isOpen={isOpen}
            onClose={onClose}
            ariaLabelledBy="payment-verification-title"
            ariaDescribedBy="payment-verification-description"
        >
            <div className="p-6 space-y-6">
                <div>
                    <h2 id="payment-verification-title" className="text-xl font-bold text-gray-900 mb-2">
                        Verify Payment Claim
                    </h2>
                    <p id="payment-verification-description" className="text-sm text-gray-600">
                        Review and verify the payment claim for invoice {paymentClaim.invoiceReference}
                    </p>
                </div>
                ...
                {showRejectForm && (
                    <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                        ...
                        <AccessibleFormField
                            ...
                            required
                            type="text"
                            error={error && !customRejectionReason ? error : undefined}
                            helpText="Enter the reason for rejection."
                            ariaDescribedBy={error ? 'rejection-reason-error' : 'rejection-reason-help'}
                            className="mb-2"
                        />
                        {rejectionReason === 'other' && (
                            <AccessibleFormField
                                ...
                                onChange={setCustomRejectionReason}
                                required
                                type="textarea"
                                error={error && customRejectionReason === '' ? error : undefined}
                                helpText="Describe why you're rejecting this claim."
                                ariaDescribedBy={error ? 'custom-reason-error' : 'custom-reason-help'}
                                className="mb-2"
                            />
                        )}
                    </div>
