...
export function DashboardInvoiceTable() {
    ...
    useEffect(() => {
        async function fetchInvoices() {
            ...
            // Emit analytics event for invoice table view
            if (Array.isArray(data.invoices)) {
                const invoiceCount = data.invoices.length;
                // Replace 'current' with actual user ID if available
                import('@/lib/analytics/emitter').then(({ emitEvent }) => {
                    emitEvent({
                        type: 'dashboard_invoice_table_viewed',
                        userId: 'current',
                        timestamp: Date.now(),
                        status,
                        invoiceCount
    ...
    function handleSort(col: typeof sortBy) {
        ...
        // Emit analytics event for sorting
        import('@/lib/analytics/emitter').then(({ emitEvent }) => {
            emitEvent({
                type: 'dashboard_invoice_table_sorted',
                userId: 'current',
                timestamp: Date.now(),
                sortBy: col,
