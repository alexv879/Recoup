...
function verifySignature(
    ...
    if (!publicKey) {
        logWarn('SENDGRID_WEBHOOK_PUBLIC_KEY not configured, skipping signature verification');
...
async function updateEmailEvent(
    ...
    if (snapshot.empty) {
        logWarn('Email event not found for message ID', { messageId });
        return;
    }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (signature && timestamp) {
            ...
            if (!isValid) {
                logError('Invalid SendGrid webhook signature');
        ...
        logInfo(`Processing ${events.length} SendGrid webhook events`);
        ...
        for (const event of events) {
            ...
            if (!messageId) {
                logWarn('Event missing sg_message_id', { event: event.event });
                continue;
            }
            ...
            logInfo('Processed SendGrid event', {
                event: event.event,
                messageId,
                email: event.email,
        ...
        logError('SendGrid webhook processing failed', error);
