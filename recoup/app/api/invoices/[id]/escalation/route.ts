...
export async function GET(
    ...
    try {
        ...
        const now = Date.now();
        const dueDateMillis = invoice.dueDate instanceof Date
            ? invoice.dueDate.getTime()
            : (invoice.dueDate as any).toMillis ? (invoice.dueDate as any).toMillis()
                : invoice.dueDate.toDate().getTime();
        const daysOverdue = Math.floor(
            (now - dueDateMillis) / (1000 * 60 * 60 * 24)
        );

        const duration = Date.now() - startTime;
        logApiResponse('GET', `/api/invoices/${id}/escalation`, 200, {
            duration,
            userId,
        });
        ...
        logApiResponse('GET', `/api/invoices/${id}/escalation`, error.statusCode || 500, {
            duration,
        });
