...
import { UnauthorizedError, BadRequestError, NotFoundError, ForbiddenError } from '@/utils/error';
...
export async function POST(
    ...
    try {
        ...
        logApiRequest('POST', `/api/invoices/${id}/escalation/pause`, {
            userId,
            action,
            reason,
        });
        ...
        if (!['pause', 'resume'].includes(action)) {
            throw new BadRequestError('Invalid action. Must be "pause" or "resume"');
        }
        ...
        if (action === 'pause') {
            ...
            // Track analytics
            await trackEvent('escalation_paused', {
                invoice_id: id,
            ...
            logApiResponse('POST', `/api/invoices/${id}/escalation/pause`, 200, {
                duration,
                userId,
            });
            ...
            // Track analytics
            await trackEvent('escalation_resumed', {
                invoice_id: id,
                invoice_reference: invoice.reference,
                reason: resumeReason,
                freelancer_id: userId,
        ...
        logApiResponse('POST', `/api/invoices/${id}/escalation/pause`, error.statusCode || 500, {
            duration,
        });
