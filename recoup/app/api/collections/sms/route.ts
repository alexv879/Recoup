...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // ✅ SECURITY FIX: Rate limiting for expensive SMS endpoints
    const rateLimit = await rateLimiters.sms.check(userId);
    if (!rateLimit.allowed) {
      return NextResponse.json(
        {
          error: 'Rate limit exceeded for SMS. Please try again later.',
